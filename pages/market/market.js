...
Page({
  ...
  data: {
    listData: [],
    isPermissed: wx.getStorageSync("isPermissed"),
    marketData: [],
    tmpData: [],
    reachCount: 0,
    isEnd: false,
    ...
    TabCur: 0,
    scrollLeft: 0,
    types: ['全部', '电脑数码', '家用电器', '运动', '服饰鞋包', '个护化妆', '日用百货', '食品保健', '图书音像', '文化娱乐', '文具用品'],
    selectedIndex: 0,
    col1: [],
    col2: [],
    widthcoefficient: 0,
  ...
  tapCollect: function(e) {
    ...
    } else if (flag && this.data.isPermissed) {
      let objectId = -1;
      if (e.target.dataset.loc == "left") {
        let goodIndex = -1;
        for (let i = 0; i < this.data.col1.length; i++) {
    ...
    this.setData({
      col1: this.data.col1,
      col2: this.data.col2,
      isPermissed: this.data.isPermissed
    })
  },
  /**
   * 点击卡片查看商品详情
  ...
  onTabItemTap(item) {
    let that = this;
    let isPermissed = wx.getStorageSync("isPermissed");
    //保证只有在第一次授权后才页面重新更新页面，否则不更新
    if (that.data.hasChanged != isPermissed && !that.data.isPermissed) {
      resetDatas(that);
      wx.switchTab({
        url: '../market/market',
        success: function(e) {
        ...
        if (keyword.length != 0) {
          for (let i = 0; i < res.length; i++) {
            if (res[i].attributes.g_name.indexOf(keyword) != -1) {
              ...
              console.log(res[i].attributes.g_name)
            }
